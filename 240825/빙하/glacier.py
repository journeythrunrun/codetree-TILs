...
while(1):
    glaciers=deque() # - 다음 녹이기 '타임 마다' 타겟된(이미 저번에 녹아진) 얼음 '리셋' # - 녹임
    bfs((0,0)) # - bfs 녹일 대상 탐색

    # - '처음부터 다 0인 케이스' 있다면 : 첫 bfs탐색 후 타겟인 얼음 길이가 0이 아닌 경우 체크 후[탈출 후]에 t 증가시키기
    if len(glaciers)==0: 
        break
    # - t :         ~ 여기까지
    ...
    # - t : 여기부터~ # 종료 체크 하고 나서 t를 증가 시키기 때문에
    t+=1 # - 't 증가 _ 여기 부터 이번 시간' '. while 첫 줄 기준 아님  
    
    # - '변하는 변수 관련해서 필요한 값'(한 번에 녹인 빙하의 개수) '있을 때 저장'해놓기 # 종료 체크 하고 나서 t를 증가 시키기 때문에, 종료 기준으로 전인 여기의 길이가 마지막 길이.  ((여기까지 왔다는 것은, 이번 타임에 녹일 빙하의 길이가 0이 아닐 때라 이전 길이))
    old_len=len(glaciers) # - 녹이는 코드 전에 저장 # - '큰 흐름들'(이 while문의 핵심 흐름 코드 순서) 먼저 잡고 하기?

    q=glaciers.copy() # - queue copy
...
#   + 실수 : 조건문 ==, True오타 , append 때 괄호빼먹 ( (x ,y) )

# - ! nxm을 mxn으로 알았다가 '특정 부분 수정'했으면, '전체 코드 부분 다 체크'하면서 보기