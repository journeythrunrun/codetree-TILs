...
def search (recent_index,cnt):
    ...
    if cnt==m : # 1개면 바로 True라 ^ 연산 시도도 안됨
        ...
        # -> (최종) 저장해나갈 때 병렬 요소 추가
        # ->  그것도 귀찮으니, 영향 없는(뇌로 암산) 시간복잡도 정도_ 새저장소 1개에 append 후 나중에 연산 -> But, [append('리스트')해버리면 주소참조라, '변경되는 리스트면 같이 변경돼서' 귀찮아짐. 대신 주소 참조하며 그런 방향으로 전역화 잘됐던 거일 수도  # - 다른 변수의 순서, 추가, 전역화 : tuple 순서 있음 | (a=(3,) )== (a=tuple([3]_iterable) ) | 불변형이긴 한데 a=([3,3],) -> a+=([4,4],) | 순서 있음 | BUT 지역변수임,  str 동일 ] 
        return 
