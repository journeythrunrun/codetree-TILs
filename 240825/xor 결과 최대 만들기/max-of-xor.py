...
# - 라이브러리, DP로 구현 다 하기  
#   + i. 시간복잡도 *r 정도는 괜찮으면 -> 라이브러리 사용_코딩속도 개이득 
#   + ii. /r 의의있으면 : DP로 구현
...
# - 1h 29m 
# - O(nCr) _ not *r인 문제. 전체 comb 세트 생성 및 '각 r 개의 요소 저장' 아니고, 해당 위치에서 연산이라서임
#   > 각 세트들이 있는 전체 세트를 생성하려면  O( r*nCr)_라이브러리, DP 구현. < -문제에 따라 -> 전체 세트를 생성하지 않고, 각 세트들에서 저장안하고 바로 연산하면 되는 거면 O(nCr) _ DP 구현
