...
for i in range(t): ## 초
    ...
    for jj in range(len(coins)): 
        if jj in dele : # 이미 사라진 애들은 continue! # 데이터 아예 삭제(m2)pop이나 remove)해버지 않고, (m1)따로 저장하는 거면 해당 중복위치 탈출 조건문 필요
            continue
        for kk in range(jj+1, len(coins)):
            if kk in dele:
                continue
            if coins[jj] == coins[kk] : ## pop 하면 for j , k 밀려서 m1) add방법씀
                dele.add(jj) 
                dele.add(kk)   
...
#   + [2] 중복 위치 : 이미 제거된 애들이랑은 ((상쇄를 위한))비교하면 안되는데, 그게 고려 안됐네
# - break <-> continue 주의해라